...
const fileParserService = new fileParserService_1.default();
const upload = (0, multer_1.default)({
    storage: multer_1.default.memoryStorage(),
    limits: {
        fileSize: fileParserService_1.default.getMaxFileSize(),
    },
    fileFilter: (req, file, cb) => {
        if (fileParserService_1.default.isSupportedFileType(file.originalname, file.mimetype)) {
            cb(null, true);
        }
        else {
...
app.post('/api/ai/parse-quiz', upload.single('file'), async (req, res) => {
    try {
        ...
        if (uploadedFile) {
            console.log(`处理上传文件: ${uploadedFile.originalname}, 大小: ${fileParserService_1.default.formatFileSize(uploadedFile.size)}`);
            const parseResult = await fileParserService.parseFile(uploadedFile.buffer, uploadedFile.originalname, uploadedFile.mimetype);
...
function generateDemoQuizHtml(fileName, content, order) {
    const title = fileName.replace(/\.[^/.]+$/, "");
    const wordCount = content.trim().split(/\s+/).length;
    const estimatedQuestions = Math.min(Math.max(Math.floor(wordCount / 20), 3), 10);
    return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
app.listen(PORT, () => {
    ...
    console.log(`📏 文件大小限制: ${fileParserService_1.default.formatFileSize(fileParserService_1.default.getMaxFileSize())}`);
});
